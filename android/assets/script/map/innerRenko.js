...
black.addAction(Actions.sequence(Actions.fadeOut(0.3),Actions.run(function(){CG.dispose(black)})));	
	...
	CG.push(tip1.color(1,1,1,0).oranCenter().position((GameUtil.screen_width/2-tip1.getWidth()/2), (GameUtil.screen_height/2-+tip1.getHeight()/2)));
	CG.addAction(Actions.color(new Color(1,1,1,1),0.3));
	say("   ");			
	CG.dispose(tip1);
	CG.push(tip2.oranCenter().position((GameUtil.screen_width/2-tip2.getWidth()/2), (GameUtil.screen_height/2-+tip2.getHeight()/2)));
	say("   ");
	tip2Action(Actions.sequence(Actions.fadeOut(0.3),Actions.run(function(){CG.dispose(tip2)})));
