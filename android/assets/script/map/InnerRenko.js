...
if(RPG.getFlag("1-1-inner")==null){
	...
	RPG.ctrl.cg.push(black=(Image) $.add(Res.getNP(Setting.UI_BASE_IMG)).setSize(GameUtil.screen_width, GameUtil.screen_height).setColor(Color.BLACK).getItem()).addAction(Actions.sequence(Actions.fadeOut(0.3f),Actions.run(RPG.ctrl.cg.dispose(black));
	...
	say("   ");
	tip2.addAction(Actions.sequence(Actions.fadeOut(0.3f),Actions.run(RPG.ctrl.cg.dispose(tip2))));
	black.addAction(Actions.sequence(Actions.color(new Color(0,0,0,0),0.3f),Actions.run(RPG.ctrl.cg.dispose(black));
	setKeyLocker(false);				
	}			
removeSelf();
end();
