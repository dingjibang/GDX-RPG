...
//				RPG.ctrl.hero.setVisible(false);
//			}});
//			setKeyLocker(true);
...
//			and(setBalloon(yuki, BalloonType.沉默));
			...
			say("（好吧……）","梅莉");
			hideMSG();
			and(setBalloon(yuki,BalloonType.疑惑));
			wait(60);
			showMSG(莲子);
			say("啊……实际上是这样的","莲子");
			...
			$(new BaseScriptExecutor() {public void init() {
				if(currentSelect("不知道怎么决定")){
					_$(showMSG(莲子));
					_$(say("诶，自己不能决定吗","莲子"));
					_$(say("那就抛硬币试试吧，命运的抉择交给命运决定！\n正面就一起来，背面的话结成同学就在家等我们好了","莲子"));
					_$(hideMSG());
					_$(playSE("coinfalling.mp3"));
					_$(Instance.this.wait(250));
					_$(showMSG(莲子));
					_$(say("唔，是正面呢","莲子"));
					_$(say("那么结成同学就一起来吧","莲子"));
					_$(showMSG(梅莉));
					_$(say("到时候可能会出现无法预料的情况，我们也无法顾及结成同学的安全，请你照顾好自己哦！ ","梅莉"));
				}else{
					_$(showMSG(莲子));
					_$(say("不愧是自己找到密封俱乐部的人啊，果然要一起来吗","莲子"));
					_$(showMSG(梅莉));
					_$(say("不过出了危险的话，我们也不能保证保护你的安全","梅莉"));
					_$(say("到时候不要后悔哦","梅莉"));
				}
			}});
			showMSG(莲子);
			...
			hideMSG();
			$(new BaseScriptExecutor() {public void init() {
				RPG.ctrl.cg.push(black=(Image) $.add(Res.getNP(Setting.UI_BASE_IMG)).setSize(GameUtil.getScreenWidth(), GameUtil.getScreenHeight()).setColor(0,0,0,0).getItem()).addAction(Actions.color(new Color(0,0,0,1),1f));
			}});
			wait(60);
			Move.teleportAnotherMap(Instance.this,  "inner.tmx",7,18,3);
			removeSelf();
