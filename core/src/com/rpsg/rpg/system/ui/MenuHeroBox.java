...
public class MenuHeroBox extends Group {
	...
	public MenuHeroBox(Hero hero) {
		...
		addActor(lv=Res.get(Setting.UI_BASE_IMG).size(70, 17).disableTouch().color(hero.getProp("dead").equals(Target.TRUE)?Color.valueOf("666666"):new Color(1,1,1,0.9f)));
		addActor($.add(Res.font.getLabel(hero.getProp("level")+"",15)).setColor(0, 0, 0, 1).setPosition(32, 0).setAlign(Align.center).getItem());
		addActor($.add(Res.get(Setting.UI_BASE_IMG)).setSize(0,4).setY(-4).setColor(hero.getProp("dead").equals(Target.TRUE)?Color.valueOf("424242"):HPColor).addAction(Actions.sizeTo((float)hero.getProp("hp")/(float)hero.getProp("maxhp")*70, 4,0.8f,Interpolation.pow4)).getItem());
		addActor($.add(Res.get(Setting.UI_BASE_IMG)).setSize(0,4).setY(-8).setColor(hero.getProp("dead").equals(Target.TRUE)?Color.valueOf("505050"):MPColor).addAction(Actions.sizeTo((float)hero.getProp("mp")/(float)hero.getProp("maxmp")*70, 4,0.8f,Interpolation.pow4)).getItem());
