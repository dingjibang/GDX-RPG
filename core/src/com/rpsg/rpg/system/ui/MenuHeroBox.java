...
public class MenuHeroBox extends Group {
	...
	public MenuHeroBox(Hero hero) {
		...
		addActor(lv=Res.get(Setting.UI_BASE_IMG).size(70, 17).disableTouch().color(hero.target.getProp("dead")==(Target.TRUE)?Color.valueOf("666666"):new Color(1,1,1,0.9f)));
		...
		addActor($.add(Res.get(Setting.UI_BASE_IMG)).setSize(0,4).setY(-4).setColor(hero.target.getProp("dead")==(Target.TRUE)?Color.valueOf("424242"):HPColor).addAction(Actions.sizeTo((float)hero.target.getProp("hp")/(float)hero.target.getProp("maxhp")*70, 4,0.8f,Interpolation.pow4)).getItem());
		addActor($.add(Res.get(Setting.UI_BASE_IMG)).setSize(0,4).setY(-8).setColor(hero.target.getProp("dead")==(Target.TRUE)?Color.valueOf("505050"):MPColor).addAction(Actions.sizeTo((float)hero.target.getProp("mp")/(float)hero.target.getProp("maxmp")*70, 4,0.8f,Interpolation.pow4)).getItem());
