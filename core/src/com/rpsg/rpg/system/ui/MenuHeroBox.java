...
public class MenuHeroBox extends BGActor {
	...
	public MenuHeroBox(Hero hero) {
		...
		actors.add($.add(Res.get(Setting.UI_BASE_IMG)).setSize(0,4).setY(-4).setColor(HPColor).addAction(Actions.sizeTo((float)hero.prop.get("hp")/(float)hero.prop.get("maxhp")*70, 4,0.55f)).getItem());
		actors.add($.add(Res.get(Setting.UI_BASE_IMG)).setSize(0,4).setY(-8).setColor(MPColor).addAction(Actions.sizeTo((float)hero.prop.get("mp")/(float)hero.prop.get("maxmp")*70, 4,0.55f)).getItem());
