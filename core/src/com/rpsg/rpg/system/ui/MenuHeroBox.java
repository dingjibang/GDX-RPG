...
public class MenuHeroBox extends BGActor {
	...
	public MenuHeroBox(Hero hero) {
		this.hero=hero;
		this.setSize(70, 70);
		image.setColor(Color.valueOf(hero.prop.get("dead").equals(Hero.TRUE)?"555555":hero.color));
		actors.add($.add(new HeroImage(hero)).setPosition(10, 5).getItem());
		actors.add(lv=Res.get(Setting.UI_BASE_IMG).size(70, 17).disableTouch().color(hero.prop.get("dead").equals(Hero.TRUE)?Color.valueOf("666666"):new Color(1,1,1,0.9f)));
