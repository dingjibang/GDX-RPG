	...
	public ZoomBox(Image image, int width, int height) {
		...
		addListener(new InputListener() {
			...
			public boolean scrolled(InputEvent event, float x, float y, int amount) {
				...
				float x1 = img.getX();
				float y1 = img.getY();
				float ox = amount * rate * img.getWidth() / 2;
				float oy = amount * rate * img.getHeight() / 2;
				img.setPosition(x1 - ox, y1 - oy);
		...
		Gdx.input.setInputProcessor(new GestureDetector(new GestureListener() {
			...
			public boolean zoom(float initialDistance, float distance) {
				...
				float x1 = img.getX();
				float y1 = img.getY();
				float ox = distance / 100 * rate * img.getWidth() / 2;
				float oy = distance / 100 * rate * img.getHeight() / 2;
				img.setPosition(x1 - ox, y1 - oy);
				img.setWidth(img.getWidth() * f);
				img.setHeight(img.getHeight() * f);
				return false;
