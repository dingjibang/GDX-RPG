...
public class TileRenderer implements TiledMapRenderer, Disposable {
	...
	public TileRenderer (TiledMap map) {
		this(map, 1, 2000);
	}

	/** Creates a renderer with a cache size of 2000. */
	public TileRenderer (TiledMap map, float unitScale) {
		this(map, unitScale, 2000);
	}

	/** @param cacheSize The maximum number of tiles that can be cached. */
	public TileRenderer (TiledMap map, float unitScale, int cacheSize) {
		this.map = map;
	...
	public void renderTileLayer (TiledMapTileLayer layer) {
		final float color = getSpriteCache().getColor().toFloatBits();
