...
public class SupportView extends SidebarView {
	...
	public void init() {
		...
		if(hero.support!=null){
			$.add(hero.defaultFG().position(1150, 0).scaleY(0.35f).color(1,1,1,.5f).scaleX(-0.35f)).setOrigin(Align.bottomLeft).appendTo(group);
			$.add(Res.get(hero.jname,43)).setPosition(540, 440).setColor(1, 1, 1, .2f).appendTo(group);
			$.add(Res.get("角色等级"+hero.target.getProp("level")+"，社群("+hero.association.name+")等级"+hero.association.level,36)).setPosition(430, 380).setColor(1, 1, 1, .7f).appendTo(group);
