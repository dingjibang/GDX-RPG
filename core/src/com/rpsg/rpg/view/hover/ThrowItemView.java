...
public class ThrowItemView extends SidebarView{
	...
	public void init() {
		...
		group.addActor(Res.get(icon.baseItem.name, 38).position(575, 365).width(330).overflow(true).color(Color.valueOf("ff6600")));
		group.addActor(Res.get("持有 "+icon.baseItem.count+" 个", 22).position(575, 330).width(330).overflow(true));
		group.addActor(counter = new Counter(icon.baseItem.count).position(435, 120));
