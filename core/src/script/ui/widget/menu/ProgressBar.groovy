...
class ProgressBar extends Group{
	...
	ProgressBar(String color, int width, int height, int progress, int max, int padding = 2) {
		...
		inner2 = $("base").a(0.2).position(padding, (float)(getHeight() / 2)).height((float)(getHeight() / 2 - padding)) to this
	...
	ProgressBar progress(int progress){
		...
		inner.stopActions().action(Actions.sizeTo(width, (float)(getHeight() - padding * 2), 0.4f, Interpolation.pow4Out))
		inner2.stopActions().action(Actions.sizeTo(width, (float)(getHeight() / 2 - padding), 0.4f, Interpolation.pow4Out))
		this
	}
